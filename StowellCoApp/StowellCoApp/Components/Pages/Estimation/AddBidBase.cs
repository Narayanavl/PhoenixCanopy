...
{
    public class AddBidBase : ComponentBase
    {
        ...
        [Inject]
        private ILogger<AddBidBase> logger { get; set; } = default!;
        ...
        public List<DivisionOption> divisions = new()
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                ...
                txtJobId = await _estimationService.GetNewBidNumber();
        ...
        protected async Task SaveBidsClick()
        {
            ...
            var BidNumber = txtJobId.ToString();//await _estimationService.GetNewBidNumber();
            _bidAmount ??= new BidAmount();
            _bidAmount.Id = 0;
        ...
        protected List<string> departments = new()
    {
        ...
        public async Task SubmitBidAsync()
        {
            try
            {
                ...
                var response = await _estimationService.SubmitBid(_bidInfoDto);
                isLoading = false;
                StateHasChanged(); // force UI update
                if (response != null && response.Success)
                {
                    ...
                    //    Detail = true ? "Project Details Saved Successfully" : "Error while submitting project details",
                    //    //Duration = 10000,
            ...
            new AlertOptions() { OkButtonText = "Ok" }
               ...
               //new AlertOptions() { OkButtonText = "Ok" });
                ...
                else
                {
                    ...
                    //               Summary = "Error",
                    //               Detail = string.IsNullOrWhiteSpace(response?.Message)
                    //? "Error while submitting project details"
                    //: response.Message,
                    //               Duration = 4000
                    //           });
                    //var confirmed = await DialogService.Alert(
                    //       "Error while submitting project details",
                    //       "Error",
                    //       new ConfirmOptions() { OkButtonText = "Ok" });

                    //if (confirmed == true)
                    //{
                    //   // NavigationManager.NavigateTo("/Estimation/BidQueue", forceLoad: true);
                    //}
                    await DialogService.Alert("Error while saving project details.", "Error");
                }
            }
            catch (Exception ex)
            {
